...
**Kubuno Jarvis — assistant IA multi-modèles (Ollama local, agents, RAG)**

A module for [Kubuno](https://github.com/kubuno/core), the self-hosted, libre (AGPLv3) cloud platform.

## Architecture

A standalone Rust process that registers with the [core](https://github.com/kubuno/core) at startup; the core proxies its routes (`/api/v1/jarvis/*`) and serves its runtime-loaded React frontend bundle.

- **Backend** — `src/`: Axum + SQLx (PostgreSQL, schema `jarvis`); migrations in `migrations/`.
...
```bash
cargo build --release                     # → target/release/kubuno-jarvis
cd frontend && npm ci && npm run build     # → dist/{entry.js, entry.css}
bash build_deb.sh                          # → dist/kubuno-jarvis_*.deb
```
